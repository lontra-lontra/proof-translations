...
\begin{definition} \textbf{Translation in classical logic}
    ...
    \begin{itemize}
        ...
        $f(\sigma,r)=f_{Rule}(\sigma,r)=r_0$, and so  $\mathcal{T}_p(\tau) = \RightLabel{\scriptsize{Rule on  $\sigma$}}\AxiomC{$\mathcal{T}_p(\tau_0)$}\UnaryInfC{$\mathcal{T}(r)$}\DisplayProof$, and we know it to be a valid one 
        by the last theorem
        \item if $r$ has two children $r_1$ and $r_2$ with corresponding subtrees $\tau_1$ and $\tau_2$:\\ by definition there exists a sentence $\sigma$  such that
            ...
            $\mathcal{T}_p(\tau) = \RightLabel{\scriptsize{Rule on $\sigma$}}\AxiomC{$\mathcal{T}_p(\tau_1)$}\AxiomC{$\mathcal{T}_p(\tau_2)$}\BinaryInfC{$\mathcal{T}(r)$}\DisplayProof$ , and we know it to be a valid one 
            by the last theorem
    \end{itemize}
\end{definition}
}
